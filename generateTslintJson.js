...
try {
  ...
  process.stderr.write(`Cannot convert to JSON : ${err.message  || err}\n`);
  return process.exit(1);
}

fs.writeFile(path.join(__dirname, 'tslint.json'), json, (err) => {
  if (err) {
    process.stderr.write(`Cannot update tslint.json : ${err.message  || err}\n`);
    return process.exit(1);
  }
  process.stdout.write(`tslint.json generated\n`);
  process.exit(0);
});
